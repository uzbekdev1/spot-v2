...
namespace ClientApi.Controllers
{
    public class AccountController : BaseController
    {
        ...
        public AccountController(SpotService spotService, CryptographyHelper cryptographyHelper, IConfiguration configuration, ILogger<AccountController> logger)
        {
        ...
        public async Task<IActionResult> Login([FromBody] LoginEmbed model)
        {
            try
            {
                ...
                var info = await _spotService.GetUser(result.login, result.password, result.uid);
                ...
                _logger.LogInformation($"User info: {JsonConvert.SerializeObject(info, Formatting.None)}");
            ...
            {
                _logger.LogError(LogGenerate.Instance.GenerateLogError("", exp.Message, exp.InnerException, exp.StackTrace, new { model }));
